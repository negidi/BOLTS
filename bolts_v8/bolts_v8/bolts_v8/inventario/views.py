...
def registrarCliente(request):
    ...
    clientes.objects.create(id_compra = idco,rut_cliente = rut_clte, nombre_cliente = nombre_clte, telefono_contacto =telefono_clt, direccion = direccion_clte, unidades = unidad,maquinaria = z,total = total)
...
''' se comienza a crear la funcion para agregar el filtro de clientes
def filtroclnte(request):
    if request.GET["nombre_cltne"]:
        clientes = request.GET["nombre_cltne"]
        x = clientes.objects.filter(nombre_cliente__iexact=clientes)
    
        
        return render(request,"listarherramientas.html",{"fullclte": x,"query":clientes})
